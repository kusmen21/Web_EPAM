...
public class DenyRequestCommand  extends ExtendedСommand {
	private static Logger log = LogManager.getLogger(DenyRequestCommand.class);
	private static final String UNKNOWN_ERROR = "error.unknown_error";
	private static final String ERROR_ID_EMPTY = "error.id_empty";
	private static final String INFO_REQUEST_DENIED = "info.request_denied";
		...
		try {
			if (requestId != null && !requestId.isEmpty()) {
				...
				if (administrator != null) {
					...
					boolean result = adminService.denyRequest(id, adminId);
					if (!result) {
					...
					request.getSession().setAttribute(ADMIN_INFO_ATTRIBUTE,
							MessageManager.getProperty(INFO_REQUEST_DENIED));
				} else {
